...
def _write_jinja_file(template_file, template_vars, file_path, overwrite=True):
    ...
    template_loader = FileSystemLoader(searchpath=os.path.join(dirname, "dss", "templates"))
...
def generate_python_bindings(test_api_path=None):
    ...
    dirname = os.path.dirname(__file__)
    api_function_path = os.path.join(dirname, "dss", "__init__.py")
    autogen_for_composite_commands_path = os.path.join(dirname, "dss", "composite_commands", "__init__.py")
    for path in api_function_path, autogen_for_composite_commands_path:
        if os.path.exists(path):
            os.remove(path)
            open(path, 'a').close()

    autogenerated_path = os.path.join(dirname, "dss", "autogenerated")
    ...
    for endpoint_name in sorted(param_holders.keys()):
        ...
        file_path = os.path.join(dirname, "dss", "autogenerated", file_name)
    ...
    file_path = os.path.join(dirname, "dss", "__init__.py")
