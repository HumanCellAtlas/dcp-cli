...
def generate_python_bindings(test_api_path=None):
    ...
    autogen_for_abstractions_path = os.path.join(dirname, "api", "abstractions", "__init__.py")
    for path in (api_function_path, autogen_for_abstractions_path):
    ...
    # Write the initial api functions to api init file because abstractions rely on these.
    ...
    # Need to autogenerate essentially the same bindings for the abstraction functions to utilize
    # to avoid circular imports.
    template_file = "/abstraction_functions.jinja"
    autogen_for_abstractions_path = os.path.join(dirname, "api", "abstractions", "__init__.py")
    ...
    abstractions = importlib.import_module("hca.api.abstractions")
    ...
    for importer, modname, _ in pkgutil.iter_modules(abstractions.__path__, prefix):
        ...
        for class_name, class_obj in clsmembers:
            ...
            template_vars = {'class_name': class_name,
                             ...
                             'endpoint_info': endpoint_info}
            function_payload['classes'].append(template_vars)
            import_ = "from .abstractions.{} import {}".format(snake_command_name, class_name)
